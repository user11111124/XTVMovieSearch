            ...
            for file in files:
                ...
                await bot.send_cached_media(
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [[
                            InlineKeyboardButton('ᴋᴅʀᴀᴍᴀʜᴜʙ', url=GRP_LNK),
                            InlineKeyboardButton('ᴄʜᴀᴛ', url=CHNL_LNK)
                        ],[
    ...
    if settings["imdb"]:
        ...
        if IMDB_CAP:
            ...
            if imdb:
                ...
                for file in files:
                    cap += f"<b>📁 <a href='https://telegram.me/{temp.U_NAME}?start=files_{file['file_id']}'>[{get_size(file['file_size'])}] {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file['file_name'].split()))}\n\n</a></b>"
                cap += f"<blockquote>⚠️ ᴛʜɪꜱ ᴍᴇꜱꜱᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴅᴇʟᴇᴛᴇᴅ ᴀꜰᴛᴇʀ 5 ᴍɪɴᴜᴛᴇꜱ 🗑️</blockquote>"
    else:
        cap = f"📂 ʜᴇʀᴇ ɪ ꜰᴏᴜɴᴅ ꜰᴏʀ ʏᴏᴜʀ sᴇᴀʀᴄʜ:- {search}\n\n</b>"
        for file in files:
            cap += f"<b>📁 <a href='https://telegram.me/{temp.U_NAME}?start=files_{file['file_id']}'>[{get_size(file['file_size'])}] {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file['file_name'].split()))}\n\n</a></b>"
        cap += f"<blockquote>⚠️ ᴛʜɪꜱ ᴍᴇꜱꜱᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴅᴇʟᴇᴛᴇᴅ ᴀꜰᴛᴇʀ 5 ᴍɪɴᴜᴛᴇꜱ 🗑️</blockquote>"
    return cap
