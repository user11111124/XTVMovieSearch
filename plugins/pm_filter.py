        ...
        if manual == False:
            ...
            try:
                if settings['auto_ffilter']:
                    ai_search = True
                    reply_msg = await message.reply_text(f"<b>ꜱᴇᴀʀᴄʜɪɴɢ ꜰᴏʀ {message.text} 👀</b>")
...
async def pm_text(bot, message):
    ...
    if PM_SEARCH == True:
        ai_search = True
        reply_msg = await bot.send_message(message.from_user.id, f"<b>ꜱᴇᴀʀᴄʜɪɴɢ ꜰᴏʀ {content} 👀</b>", reply_to_message_id=message.id)
    ...
    try:
        if int(query.from_user.id) not in [query.message.reply_to_message.from_user.id, 0]:
            return await query.answer(
                f"⚠️ ʙᴀᴋᴀ! {query.from_user.first_name},\nᴛʜɪꜱ ɪꜱ ɴᴏᴛ ꜰᴏʀ ʏᴏᴜ 🤡",
                show_alert=True,
            )
    except:
        pass
...
async def seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.insert(
        ...
        [
            InlineKeyboardButton(
                text="↡ ꜱᴇʟᴇᴄᴛ ꜱᴇᴀꜱᴏɴꜱ ↡", callback_data="ident"
            )
        ],
    )
    req = query.from_user.id
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        if not free_trial_status:            
            ...
            new_text = "<b>ʏᴏᴜ ᴄᴀɴ ᴜsᴇ ꜰʀᴇᴇ ᴛʀɪᴀʟ ꜰᴏʀ 5 ᴍɪɴᴜᴛᴇs ꜰʀᴏᴍ ɴᴏᴡ 😀\n\nआप अब से 5 मिनट के लिए निःशुल्क ट्रायल का उपयोग कर सकते हैं 😀</b>"        
            await query.message.edit_text(text=new_text)
            return
        else:
            new_text= "<b>🤡 ʏᴏᴜ ʜᴀᴠᴇ ᴀʟʀᴇᴀᴅʏ ᴜꜱᴇᴅ ꜰʀᴇᴇ ᴛʀɪᴀʟ, ɴᴏ ᴍᴏʀᴇ ꜰʀᴇᴇ ᴛʀᴀɪʟ. ᴘʟᴇᴀꜱᴇ ʙᴜʏ ꜱᴜʙꜱᴄʀɪᴘᴛɪᴏɴ ʜᴇʀᴇ ᴀʀᴇ ᴏᴜʀ ↠ /plans**"
            await query.message.edit_text(text=new_text)
            return
        ...
        btn = [[            
            InlineKeyboardButton("sᴇɴᴅ ʏᴏᴜʀ ᴘᴀʏᴍᴇɴᴛ ʀᴇᴄᴇɪᴘᴛ ʜᴇʀᴇ🧾", url = 'https://t.me/inocent_aman')
        ]]
        btn.append(
    ...
    if query.data.startswith("file"):
        ...
        try:
            if settings['is_shortlink'] and not await db.has_premium_access(query.from_user.id):
                if clicked == typed:
                    ...
                    await query.answer(f"⚠️ ʙᴀᴋᴀ! {query.from_user.first_name},\nᴛʜɪꜱ ɪꜱ ɴᴏᴛ ꜰᴏʀ ʏᴏᴜ 🤡", show_alert=True)
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("Jᴏɪɴ ᴏᴜʀ ᴄʜᴀɴɴᴇʟ ᴍᴀɴ! 😒", show_alert=True)
            return
     ...
     #   if not search:
      ...
      #      return
        ...
        files, n_offset, total = await get_search_results(query.message.chat.id, search, offset=int(offset), filter=True)
        await send_all(client, query.from_user.id, files, ident, query.message.chat.id, query.from_user.first_name, query)
        await query.answer(f"Hey {query.from_user.first_name}, ᴀʟʟ ꜰɪʟᴇꜱ ᴏɴ ᴛʜɪꜱ ᴘᴀɢᴇ ʜᴀꜱ ʙᴇᴇɴ ꜱᴇɴᴛ ꜱᴜᴄᴄᴇꜱꜱꜰᴜʟʟʏ ᴛᴏ ʏᴏᴜʀ ᴘᴍ !", show_alert=True)
     ...
     #   if not search:
      ...
      #      return
            ...
            buttons = [
                ...
                [
                    InlineKeyboardButton('Aᴜᴛᴏ-Dᴇʟᴇᴛᴇ',
                                         callback_data=f'setgs#auto_delete#{settings["auto_delete"]}#{str(grp_id)}'),
                    InlineKeyboardButton('5 Mɪɴs' if settings["auto_delete"] else '✘ Oғғ',
                                         callbackdata=f'setgs#auto_delete#{settings["auto_delete"]}#{str(grp_id)}')
                ],
                [
                    InlineKeyboardButton('Aᴜᴛᴏ-Fɪʟᴛᴇʀ',
                                         callback_data=f'setgs#auto_ffilter#{settings["auto_ffilter"]}#{str(grp_id)}'),
        ...
        btn = [[
                InlineKeyboardButton("Aʟʀᴇᴀᴅʏ Aᴠᴀɪʟᴀʙʟᴇ", callback_data=f"unavailable#{from_user}"),
                InlineKeyboardButton("Uɴᴀᴠᴀɪʟᴀʙʟᴇ", callback_data=f"uploaded#{from_user}")
             ],[
                InlineKeyboardButton("Uᴘʟᴏᴀᴅᴇᴅ", callback_data=f"already_available#{from_user}")
              ]]
        btn2 = [[
        ...
        ident, from_user = query.data.split("#")
        btn = [[
                InlineKeyboardButton("Aʟʀᴇᴀᴅʏ Aᴠᴀɪʟᴀʙʟᴇ ✅", callback_data=f"unalert#{from_user}")
              ]]
        btn2 = [[
                 InlineKeyboardButton('Jᴏɪɴ Cʜᴀɴɴᴇʟ', url=query.message.link),
                 InlineKeyboardButton("Vɪᴇᴡ Sᴛᴀᴛᴜs", url=f"{query.message.link}")
               ]]
        if query.from_user.id in ADMINS:
            user = await client.get_users(from_user)
            ...
            await query.message.edit_text(f"<b><blockquote><strike>{content}</strike></blockquote></b>")
            await query.message.edit_reply_markup(reply_markup)
            await query.answer("Sᴇᴛ ᴛᴏ Uɴᴀᴠᴀɪʟᴀʙʟᴇ !")

            try:
                await client.send_message(chat_id=int(from_user), text=f"<b>Hᴇʏ {user.mention},\nYᴏᴜʀ ʀᴇQᴜᴇꜱᴛᴇᴅ ɪs ᴀʟʀᴇᴀᴅʏ ᴀᴠᴀɪʟᴀʙʟᴇ. Kɪɴᴅʟʏ sᴇᴀʀᴄʜ ɪɴ ᴏᴜʀ Gʀᴏᴜᴘ.</b>", reply_markup=InlineKeyboardMarkup(btn2))
            except UserIsBlocked:
                await client.send_message(chat_id=int(SUPPORT_CHAT_ID), text=f"<b>Hᴇʏ {user.mention},\nYᴏᴜʀ ʀᴇQᴜᴇꜱᴛᴇᴅ ɪs ᴀʟʀᴇᴀᴅʏ ᴀᴠᴀɪʟᴀʙʟᴇ. Kɪɴᴅʟʏ sᴇᴀʀᴄʜ ɪɴ ᴏᴜʀ Gʀᴏᴜᴘ.\n\n<blockquote>Nᴏᴛᴇ: Tʜɪs ᴍᴇssᴀɢᴇ ɪs sᴇɴᴛ ᴛᴏ ᴛʜɪs ɢʀᴏᴜᴘ ʙᴇᴄᴀᴜsᴇ ʏᴏᴜ'ᴠᴇ ʙʟᴏᴄᴋᴇᴅ ᴛʜᴇ ʙᴏᴛ. ᴛᴏ ɢᴇᴛ ᴛʜɪꜱ ᴍᴇꜱꜱᴀɢᴇ ɪɴ ʏᴏᴜʀ ᴘᴍ, ʏᴏᴜ ᴍᴜꜱᴛ ᴜɴʙʟᴏᴄᴋ ᴛʜᴇ ʙᴏᴛ.</blockquote></b>", reply_markup=InlineKeyboardMarkup(btn2))
        else:
            await query.answer("ʙᴀᴋᴀ! ᴛʜɪꜱ ɪꜱ ɴᴏᴛ ꜰᴏʀ ʏᴏᴜ. 😏", show_alert=True)

    elif query.data.startswith("uploaded"):
        try:
            # Log the raw query data to see exactly what we're getting
            print(f"Received query data: {query.data}")
            # Check if query.data contains '#' before attempting to split
            if "#" in query.data:
                parts = query.data.split("#")
                
                # Log how many parts we got from splitting and the actual split result
                print(f"Split query data into {len(parts)} parts: {parts}")
                # Ensure we have exactly two parts before proceeding
                if len(parts) == 2:
                    ident, from_user = parts
                    print(f"Processing: ident={ident}, from_user={from_user}")
                    # Validate that from_user is a valid integer (user ID)
                    try:
                        from_user = int(from_user)
                    except ValueError:
                        print(f"Error: from_user is not a valid integer. Received: {from_user}")
                        await query.answer("Invalid user ID. Please try again.", show_alert=True)
                        return
                    # Create the buttons
                    btn = [[
                        InlineKeyboardButton("⚠️ Uɴᴀᴠᴀɪʟᴀʙʟᴇ ⚠️", callback_data=f"upalert#{from_user}")
                    ]]
                    btn2 = [[
                        InlineKeyboardButton('Jᴏɪɴ Cʜᴀɴɴᴇʟ', url=query.message.link),
                        InlineKeyboardButton("Vɪᴇᴡ Sᴛᴀᴛᴜs", url=f"{query.message.link}")
                    ], [
                        InlineKeyboardButton("Rᴇᴏ̨ᴜᴇsᴛ Gʀᴏᴜᴘ Lɪɴᴋ", url="https://t.me/SasukeFilterRequest")
                    ]]
                    if query.from_user.id in ADMINS:
                        try:
                            user = await client.get_users(from_user)
                            ...
                            try:
                                await client.send_message(chat_id=from_user, text=f"<b>Hᴇʏ {user.mention},\n Sᴏʀʀʏ Yᴏᴜʀ ʀᴇQᴜᴇꜱᴛᴇᴅ ɪs ᴜɴᴀᴠᴀɪʟᴀʙʟᴇ. Sᴏ ᴏᴜʀ ꜱᴇɴᴘᴀɪ ᴄᴀɴ'ᴛ ᴜᴘʟᴏᴀᴅ ɪᴛ.</b>", reply_markup=InlineKeyboardMarkup(btn2))
                            except UserIsBlocked:
                                await client.send_message(chat_id=int(SUPPORT_CHAT_ID), text=f"<b>Hᴇʏ {user.mention},\nSᴏʀʀʏ Yᴏᴜʀ ʀᴇQᴜᴇꜱᴛᴇᴅ ɪs ᴜɴᴀᴠᴀɪʟᴀʙʟᴇ. Sᴏ ᴏᴜʀ ꜱᴇɴᴘᴀɪ ᴄᴀɴ'ᴛ ᴜᴘʟᴏᴀᴅ ɪᴛ.\n\n<blockquote>Nᴏᴛᴇ: Tʜɪs ᴍᴇssᴀɢᴇ ɪs sᴇɴᴛ ᴛᴏ ᴛʜɪs ɢʀᴏᴜᴘ ʙᴇᴄᴀᴜsᴇ ʏᴏᴜ'ᴠᴇ ʙʟᴏᴄᴋᴇᴅ ᴛʜᴇ ʙᴏᴛ. ᴛᴏ ɢᴇᴛ ᴛʜɪꜱ ᴍᴇꜱꜱᴀɢᴇ ɪɴ ʏᴏᴜʀ ᴘᴍ, ʏᴏᴜ ᴍᴜꜱᴛ ᴜɴʙʟᴏᴄᴋ ᴛʜᴇ ʙᴏᴛ.</blockquote></b>", reply_markup=InlineKeyboardMarkup(btn2))
                        except Exception as e:
                            # Log any exceptions that occur in the processing
                            print(f"Error handling user data: {e}")
                            await query.answer("An error occurred while processing the request. Please try again later.", show_alert=True)
                    else:
                        await query.answer("Yᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ sᴜғғɪᴄɪᴇɴᴛ ʀɪɢʜᴛs ᴛᴏ ᴅᴏ ᴛʜɪs !", show_alert=True)
                else:
                    print(f"Error: Unexpected number of parts after splitting: {len(parts)}. Expected 2 parts.")
                    await query.answer("Invalid data format. Please try again.", show_alert=True)
            else:
                print(f"Error: Data does not contain '#'. Received: {query.data}")
                await query.answer("Invalid request format. Please try again.", show_alert=True)
        except Exception as e:
            print(f"Unexpected error: {e}")
            await query.answer("An unexpected error occurred. Please try again later.", show_alert=True)
    elif query.data.startswith("already_available"):
        ident, from_user = query.data.split("#")
        btn = [[
            InlineKeyboardButton("✅ Uᴘʟᴏᴀᴅᴇᴅ ✅", callback_data=f"alalert#{from_user}")
        ]]
        btn2 = [[
            InlineKeyboardButton('Jᴏɪɴ Cʜᴀɴɴᴇʟ', url=query.message.link),
            InlineKeyboardButton("Vɪᴇᴡ Sᴛᴀᴛᴜs", url=f"{query.message.link}")
        ...
        if query.from_user.id in ADMINS:
            ...
            try:
                await client.send_message(chat_id=int(from_user), text=f"<b>Hᴇʏ {user.mention},\nYᴏᴜʀ ʀᴇQᴜᴇꜱᴛᴇ ʜᴀs ʙᴇᴇɴ ᴜᴘʟᴏᴀᴅᴇᴅ ʙʏ ᴏᴜʀ ꜱᴇɴᴘᴀɪ. Kɪɴᴅʟʏ sᴇᴀʀᴄʜ ɪɴ ᴏᴜʀ Gʀᴏᴜᴘ.</b>", reply_markup=InlineKeyboardMarkup(btn2))
            except UserIsBlocked:
                await client.send_message(chat_id=int(SUPPORT_CHAT_ID), text=f"<b>Hᴇʏ {user.mention},\nYᴏᴜʀ ʀᴇQᴜᴇꜱᴛᴇᴅ ʜᴀs ʙᴇᴇɴ ᴜᴘʟᴏᴀᴅᴇᴅ ʙʏ ᴏᴜʀ ꜱᴇɴᴘᴀɪ. Kɪɴᴅʟʏ sᴇᴀʀᴄʜ ɪɴ ᴏᴜʀ Gʀᴏᴜᴘ.\n\n<blockquote>Nᴏᴛᴇ: Tʜɪs ᴍᴇssᴀɢᴇ ɪs sᴇɴᴛ ᴛᴏ ᴛʜɪs ɢʀᴏᴜᴘ ʙᴇᴄᴀᴜsᴇ ʏᴏᴜ'ᴠᴇ ʙʟᴏᴄᴋᴇᴅ ᴛʜᴇ ʙᴏᴛ. ᴛᴏ ɢᴇᴛ ᴛʜɪꜱ ᴍᴇꜱꜱᴀɢᴇ ɪɴ ʏᴏᴜʀ ᴘᴍ, ʏᴏᴜ ᴍᴜꜱᴛ ᴜɴʙʟᴏᴄᴋ ᴛʜᴇ ʙᴏᴛ.</blockquote></b>", reply_markup=InlineKeyboardMarkup(btn2))
        else:
        ...
        if int(query.from_user.id) == int(from_user):
            user = await client.get_users(from_user)
            await query.answer(f"Hᴇʏ {user.first_name}, ʏᴏᴜʀ ʀᴇQᴜᴇꜱᴛ ʜᴀꜱ ʙᴇᴇɴ ᴜᴘʟᴏᴀᴅᴇᴅ !", show_alert=True)
        else:
            await query.answer("Yᴏᴜ ᴅᴏɴ'ᴛ ʜᴀᴠᴇ sᴜғғɪᴄɪᴀɴᴛ ʀɪɢᴛs ᴛᴏ ᴅᴏ ᴛʜɪs !", show_alert=True)
        ...
        if PREMIUM_AND_REFERAL_MODE == True:
            buttons = [[
                ...
                InlineKeyboardButton('ɪɴᴅᴇx', url="https://t.me/lalisaIndex"),
...
async def auto_filter(client, name, msg, reply_msg, ai_search, spoll=False):
        ...
        if len(message.text) < 100:
            ...
            if not files:
                if settings["spell_check"]:
                    ...
                    return await reply_msg.edit_text(f"**⚠️ ɴᴏ ꜰɪʟᴇ ꜰᴏᴜɴᴅ ꜰᴏʀ ʏᴏᴜʀ Qᴜᴇʀʏ - {name}**\n**ᴍᴀᴋᴇ ꜱᴜʀᴇ ꜱᴘᴇʟʟɪɴɢ ɪꜱ ᴄᴏʀʀᴇᴄᴛ.**")
        else:
            return
    else:
    ...
    if imdb:
        ...
        if settings["button"]:
            ...
            cap += f"<blockquote>⚠️ ᴛʜɪꜱ ᴍᴇꜱꜱᴀɢᴇꜱ ᴡɪʟʟ ʙᴇ ᴅᴇʟᴇᴛᴇᴅ ᴀꜰᴛᴇʀ 5 ᴍɪɴᴜᴛᴇꜱ 🗑️</blockquote>"
...
async def advantage_spell_chok(client, name, msg, reply_msg, vj_search):
    ...
    if AI_SPELL_CHECK == True and vj_search == True:
        vj_search_new = False
        vj_ai_msg = await reply_msg.edit_text("<b>ɪ ᴀᴍ ᴛʀʏɪɴɢ ᴛᴏ ꜰɪɴᴅ ʏᴏᴜʀ ᴍᴏᴠɪᴇ ᴡɪᴛʜ ʏᴏᴜʀ ᴡʀᴏɴɢ ꜱᴘᴇʟʟɪɴɢ.</b>")
        movienamelist = []
